...
- Name: Admin Schedule Management >> can manage doctor slots
- Location: c:\Users\Peter Darley\Desktop\DEV\BookSmartly\frontend\tests\admin-schedule.spec.cjs:166:3

# Error details

```
Error: locator.isVisible: Unexpected token "=" while parsing css selector "button:has-text("Slots"), text=Slots, text=Appointments, button:has-text("Manage Slots")". Did you mean to CSS.escape it?
Call log:
    - checking visibility of button:has-text("Slots"), text=Slots, text=Appointments, button:has-text("Manage Slots") >> nth=0

    at c:\Users\Peter Darley\Desktop\DEV\BookSmartly\frontend\tests\admin-schedule.spec.cjs:175:27
```
